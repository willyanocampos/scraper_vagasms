...
<body>
    <div class="header">
        ...
        <p>Mapa Interativo com Dados Reais</p>
    </div>
    ...
    <div class="container">
        <div class="stats-panel">
            <div class="stats-grid">
                ...
                <div class="stat-card">
                    <div class="stat-number">{formatted_date}</div>
    ...
    <div class="footer">
        <p>Sistema de BI - Extração de Vagas MS | Atualizado em {full_datetime}</p>
    ...
    <script>
        ...
        const companiesInfo = {companies_info_json};
        ...
        Object.keys(clusteredData).forEach(locationKey => {
            ...
            const customIcon = L.divIcon({
                html: `<div style="
                    background-color: ${mainCompany ? mainCompany.color : '#3498db'};
                    width: 35px;
                    height: 35px;
                    border-radius: 50%;
            ...
            const marker = L.marker([location.lat, location.lng], { icon: customIcon })
                .bindPopup(popupContent, { maxWidth: 500, maxHeight: 600 });
        ...
        document.getElementById('total-companies').textContent = Object.keys(companiesInfo).length;
        document.getElementById('total-locations').textContent = totalLocations;
        
        // Adiciona legenda das empresas
        const companiesLegend = document.getElementById('companies-legend');
        
        Object.keys(companiesInfo).forEach(companyId => {
            const company = companiesInfo[companyId];
            let companyJobs = 0;
            
            // Conta total de vagas da empresa
            Object.values(clusteredData).forEach(location => {
                if (location.companies[company.name]) {
                    companyJobs += location.companies[company.name].jobs.length;
                }
            });
            ...
            companyDiv.innerHTML = `
                <div class="company-icon" style="background-color: ${company.color}; color: white; text-align: center; line-height: 20px; font-size: 12px;">
                    ${company.icon}
                </div>
                <div class="company-info">
                    <div class="company-name">${company.name} (${companyJobs} vagas)</div>
                    <div class="company-desc">${company.description}</div>
                </div>
            `;
            companiesLegend.appendChild(companyDiv);
        });
        ...
        function createLocationPopup(location) {
            let html = `
                <div class="cluster-popup">
                    ...
                    <div style="text-align: center; margin-bottom: 15px; color: #7f8c8d;">
                        <strong>${location.total_jobs} vagas disponíveis</strong>
                    </div>
            `;
            
            // Para cada empresa nesta localização
            Object.keys(location.companies).forEach(companyName => {
                const company = location.companies[companyName];
                
                html += `
                    <div class="company-section" style="border-left-color: ${company.color};">
                        <div class="company-header" style="color: ${company.color};">
                            <span style="margin-right: 8px;">${company.icon}</span>
                            ${companyName} (${company.jobs.length} vagas)
                        </div>
                `;
                ...
                company.jobs.forEach((job, index) => {
                    html += `
                        <div class="job-item" style="border-left-color: ${company.color};">
                            <div class="job-title">${job.titulo}</div>
                            <div class="job-details">
                                <div style="margin: 6px 0;"><strong>📍 Local:</strong> ${job.localizacao}</div>
                                <div style="margin: 6px 0;"><strong>💼 Tipo:</strong> ${job.tipo_contrato}</div>
                                <div style="margin: 6px 0;"><strong>🏢 Modalidade:</strong> ${job.modalidade}</div>
                                <div style="margin: 6px 0;"><strong>💰 Salário:</strong> ${job.salario}</div>
                                ${job.atribuicoes && job.atribuicoes !== '' && job.atribuicoes !== 'Não informado' ? `<div style="margin: 6px 0;"><strong>📋 Atribuições:</strong><br>${job.atribuicoes}</div>` : ''}
                                ${job.requisitos && job.requisitos !== '' && job.requisitos !== 'Não informado' ? `<div style="margin: 6px 0;"><strong>🎯 Requisitos:</strong><br>${job.requisitos}</div>` : ''}
                                ${job.link && job.link !== '' ? `<div style="margin: 10px 0;"><a href="${job.link}" target="_blank" class="job-link-button">🔗 Ver Vaga Completa</a></div>` : ''}
                            </div>
                        </div>
                    `;
                });
        ...
        console.log('BI Vagas MS carregado com sucesso!');
        ...
        companies_info_json = json.dumps(self.companies, ensure_ascii=False, indent=2)
        
        html_content = html_template.replace('{clustered_data_json}', clustered_data_json)
        html_content = html_content.replace('{companies_info_json}', companies_info_json)
        html_content = html_content.replace('{formatted_date}', self.formatted_date)
        html_content = html_content.replace('{full_datetime}', self.full_datetime)
